...
export const getProfile = async (key: string, query: string, page = 1) => {
  const response = await axios.get(`https://outfit-md.herokuapp.com/profile/${query}?page=${page}`)
...
export const addProduct = async (
  ...
  const response = await axios.post('https://outfit-md.herokuapp.com/create',
    formData,
    {
      headers: {
